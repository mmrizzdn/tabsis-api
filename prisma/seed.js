...
let main = async () => {
    ...
    for (let roleData of roles) {
        ...
        if (!role) {
            console.warn(
                `Role ${roleData.name} not found, skipping permission seeding for this role`
            );
            continue;
        }
    ...
    let superadminPass = await bcrypt.hash(process.env.SUPERADMIN_PASSWORD || 'superadmin123', 10);
    ...
    await prisma.user.upsert({
        ...
        create: {
            ...
            profile: { create: { name: 'Superadmin', phoneNumber: '62123456789' } },
        },
    });
